...
docpath = ''
with open(docpath + 'els_data_strcut.json', 'r', encoding='utf-8') as f:
    doc_dict = json.load(f)

logging.basicConfig(filename=docpath + 'process_log_20240930.log',
                    level=logging.INFO,
...
for doi, art in doc_dict.items():
    ...
    #full_text = ' '.join([p['content'] for p in art['content']])

    #msg.append({"role": "user", "content": '1'})

    try:
        text_content = art['content']
        table_content = art['table']
        full = text_content + table_content


        # message = message0 + [{"role": "user", "content": text_content+table_content}]
        ...
        f=open(docpath + 'extracted_info.json', 'w', encoding='utf-8')

        json.dump(data,f, indent=4)
        f.close()
        logging.info(f"{doi}: done")
        print(f"{doi}: done")
