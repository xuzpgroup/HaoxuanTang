...
def parse_table_springer(doc):
    ...
    # get title
    div=doc.find('header',class_='u-mb-16')
    tmp = div.find('h1')
    ...
    div_table_container = doc.find('div', class_='c-article-table-container')
    if div_table_container is not None:

        tabs = div_table_container.find_all('table')
        ntab = len(tabs)
        if ntab > 1:
            message('WARNING: multiple tables in a springer html')
        elif ntab == 0:
            message('WARNING: zero table in a springer html')
            return tables
    else:
        message('WARNING: no table container found')
        return tables
...
def process_springer(path,txtpath,logpath):
    global log,iflog
    ...
    for dir0 in dirlist:
        #message(dir0)
        if os.path.isdir(path+dir0):
            filelist=os.listdir(path+dir0)
            tables=[]
            #doi=doi_dict[dir0]
            doi=dir0.replace('_','/')
            for file in filelist:
                ...
                if leng>5 and file[leng-5:leng]=='.html':    
                    message(path+dir0+'/'+file)

                    f=open(path+dir0+'/'+file,'r',encoding='utf-8')
            ...
            doc['file']=file
            doc['doi']=doi
            doc['content']=parser(path,dir0+'.html','springer',doi).sections
            doc['tables']=tables
            doc_dict[doi]=doc
    log.close()
...
logpath=path+'parse.log'
txtpath=""
doc_dict=process_springer(path,txtpath,logpath)
# process_wiley()
# process_mdpi()
f=open(path+'springer_data_strcut.json','w',encoding='utf-8')
